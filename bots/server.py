...
async def update_pinless_call(call_id, call_domain, sip_uri):
    ...
    try:
        ...
        data = {"callId": call_id, "callDomain": call_domain, "sipUri": sip_uri}
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=data, headers=headers) as response:
                if response.status >= 400:
                    ...
                    logger.error(
                        f"Error updating pinless call: {error_text}, Status: {response.status}"
                    )
                    raise HTTPException(
                        status_code=500,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update pinless call: {str(e)}"
        )
...
async def lifespan(app: FastAPI):
    ...
    logger.debug(
        f"Initializing Daily REST helper with API key: {daily_api_key[:6]}...{daily_api_key[-4:]}"
    )
...
async def start_bot(request: Request):
    try:
        ...
        logger.info(
            f"Created room: {room.url} with SIP endpoint: {room.config.sip_endpoint}"
        )
        ...
        logger.info(
            f"Starting dial-in bot with parameters: room_url={room.url}, token={token[:10]}..., call_id={call_id}, call_domain={call_domain}"
        )
        ...
        return {
            "status": "success",
            "message": "SIP call forwarded successfully",
            "room_url": room.url,
...
async def rtvi_connect(request: Request):
    ...
    try:
        ...
        logger.info("=== Starting new connection request ===")
        ...
        try:
            ...
            logger.info(
                f"Created room: {daily_room_url} with expiry in {ROOM_EXPIRY_TIME/60} minutes"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to generate room token: {str(e)}"
            )
        ...
        response_data = {"room_url": daily_room_url, "token": token}
...
async def handle_dial_in(request: Request) -> JSONResponse:
    ...
    try:
        ...
        if not call_id or not call_domain:
            logger.error("Missing callId or callDomain in request")
            raise HTTPException(
                status_code=400, detail="Missing callId or callDomain in request"
            )
    ...
    try:
        if not room_url:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=404, detail=f"Room not found: {room_url}"
                )

        logger.info(
            f"Daily room: {room.url} with SIP endpoint: {room.config.sip_endpoint}"
        )
        ...
        if not token:
            ...
            raise HTTPException(status_code=500, detail="Failed to get token for room")
        ...
        try:
            proc = subprocess.Popen(
                bot_cmd, shell=True, cwd=os.path.dirname(os.path.abspath(__file__))
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to start dial-in bot: {str(e)}"
            )
...
async def twilio_webhook(request: Request):
    ...
    try:
        ...
        logger.info(
            f"Received Twilio webhook: CallSid={call_sid}, From={from_number}, To={to_number}, Status={call_status}"
        )
        ...
        if room.config.sip_endpoint:
            ...
            logger.info(
                f"Starting Twilio dial-in bot with parameters: room_url={room.url}, token={token[:10]}..., call_sid={call_sid}"
            )
            proc = subprocess.Popen(
                [
                    sys.executable,
...
<Response>
    ...
    <Say>We're having trouble connecting your call. Please try again later.</Say>
        ...
        logger.info(
            f"Returning enhanced TwiML response to Twilio to connect to SIP endpoint: {room.config.sip_endpoint}"
        )
...
async def dial_status(request: Request):
    ...
    try:
        ...
        logger.info(
            f"Received Dial status callback: CallSid={call_sid}, DialCallStatus={dial_call_status}"
        )
...
if __name__ == "__main__":
    ...
    default_ssl_cert = os.getenv(
        "SSL_CERT", "/etc/letsencrypt/live/chatbotmailer.com/fullchain.pem"
    )
    default_ssl_key = os.getenv(
        "SSL_KEY", "/etc/letsencrypt/live/chatbotmailer.com/privkey.pem"
    )
    ...
    parser.add_argument(
        "--port-80", action="store_true", help="Run on port 80 (requires root/admin)"
    )
    ...
    parser.add_argument(
        "--ssl-key", type=str, default=default_ssl_key, help="Path to SSL key file"
    )
    parser.add_argument(
        "--no-ssl",
        action="store_true",
        default=True,
        help="Disable SSL/HTTPS (default: True)",
    )
    parser.add_argument(
        "--use-ssl", action="store_true", help="Enable SSL/HTTPS (overrides no-ssl)"
    )
    parser.add_argument(
        "--log-level",
        type=str,
    ...
    if public_ip == "0.0.0.0":
        try:
            ...
            public_ip = "127.0.0.1"
            logger.warning(
                f"Could not determine server's public IP. Using {public_ip} for URLs"
            )

    # Print Twilio webhook URL information with the detected IP
    protocol = "http" if config.no_ssl else "https"
    port_info = (
        f":{config.port}" if config.port != (80 if protocol == "http" else 443) else ""
    )
    ...
    logger.info(
        f"ℹ️ If using your IP without a domain name, ensure port {config.port} is publicly accessible"
    )

    if config.no_ssl:
        logger.info(
            f"Starting server WITHOUT SSL on http://{config.host}:{config.port}"
        )
        ...
        if host_to_use != config.host:
            logger.warning(
                f"Changed host from {config.host} to {host_to_use} to accept external connections"
            )
        ...
        if not os.path.exists(config.ssl_cert):
            ...
            print("SSL certificate not found. Running without SSL.")
            ...
            logger.warning("Falling back to HTTP mode without SSL")
            print("SSL key not found. Running without SSL.")
            ...
            logger.info(
                f"Starting server with SSL on https://{config.host}:{config.port}"
            )
